...
- **`verdict.status`** is the sign-off that used to not exist anywhere. Closed vocabulary, five values only (see above). `rewritten-by-editor` covers cases like DOSIS's "Tallet", which was rebuilt from scratch by the chefredaktør rather than lightly edited — and cases like KRAFTEN nr. 2's Sweden nuclear piece, which first landed as an unfinished editor checklist and had to be rewritten into magazine prose before it could stay in `content/`.
- **`verdict` is also where unfinished work dies.** If the piece is still a brief, a source checklist, or meta-instructions to the writer, status is `rejected` (or it never gets a path under `content/` at all). Do not mark `accepted` and “fix later.”
